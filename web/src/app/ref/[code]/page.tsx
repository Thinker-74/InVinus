...
export default async function ReferralPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const { data: prodottiRaw } = haPreferiti
    ? await prodottiQuery.in("id", preferitiIds)
    : await prodottiQuery.order("nome");

  type Prodotto = NonNullable<typeof prodottiRaw>[number] & { regioni: { nome: string } | null };
  const prodotti = (prodottiRaw ?? []) as Prodotto[];

  // Se ha preferiti, rispetta l'ordine
  const prodottiOrdinati = haPreferiti
    ? preferitiIds.map((id) => prodotti.find((p) => p.id === id)).filter(Boolean) as Prodotto[]
    : prodotti;

  const regioni = haPreferiti
    ? null // nessun raggruppamento per regione se ha selezione personale
    : [...new Set(prodottiOrdinati.map((p) => p.regioni?.nome ?? "—"))].sort();
  ...
  return (
    <div style={{ backgroundColor: "var(--color-ink)", minHeight: "100vh", color: "var(--color-pearl)" }}>
      ...
      >
        ...
        <span className="text-xs hidden sm:block" style={{ color: "var(--color-muted)" }}>Un nuovo modo di vivere il vino</span>
      </header>

      <main className="max-w-3xl mx-auto px-4 py-10 space-y-14">

        {/* Hero — profilo consulente */}
        <section className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          {/* Foto */}
          {consulente.foto_url ? (
            <img
              src={consulente.foto_url}
              alt={`${consulente.nome} ${consulente.cognome}`}
              className="w-24 h-24 rounded-full object-cover flex-shrink-0"
              style={{ border: "3px solid var(--color-gold)" }}
            />
          ) : (
            <div
              className="w-24 h-24 rounded-full flex-shrink-0 flex items-center justify-center text-3xl font-bold"
              style={{ backgroundColor: "var(--color-ash)", color: "var(--color-gold)", border: "3px solid var(--color-border)" }}
            >
          ...
          <div className="text-center sm:text-left">
            ...
            {consulente.specialita && (
              <p className="mt-1 text-sm" style={{ color: "var(--color-pearl)" }}>{consulente.specialita}</p>
            )}
            {consulente.bio && (
              <p className="mt-3 text-sm leading-relaxed" style={{ color: "var(--color-muted)" }}>{consulente.bio}</p>
            )}
          </div>
        </section>

        {/* Messaggio personale */}
        {consulente.messaggio_referral && (
          <blockquote
            className="text-base italic leading-relaxed"
            style={{ color: "var(--color-pearl)", borderLeft: "3px solid var(--color-gold)", paddingLeft: "1.25rem" }}
          >
            "{consulente.messaggio_referral}"
          </blockquote>
        )}
        ...
        <section>
          <h2 className="text-xl font-semibold mb-6" style={{ fontFamily: "var(--font-playfair)", color: "var(--color-pearl)" }}>
            {haPreferiti ? "La sua selezione" : "Il catalogo InVinus"}
          </h2>

          {haPreferiti ? (
            // Griglia piatta per selezione personale
                ...
                <div key={p.id} className="rounded-xl p-4"
                  ...
                  <div className="flex justify-between items-start gap-2">
                    <div className="min-w-0">
                      <p className="font-medium text-sm leading-snug" style={{ color: "var(--color-pearl)" }}>{p.nome}</p>
                      <p className="text-xs mt-0.5" style={{ color: "var(--color-muted)" }}>
                        {p.denominazione}{p.annata ? ` · ${p.annata}` : ""}
                      </p>
                      {p.regioni?.nome && (
                        <p className="text-xs mt-0.5" style={{ color: "var(--color-muted)" }}>{p.regioni.nome}</p>
                      )}
                    </div>
            ...
            regioni!.map((regione) => (
              <div key={regione} className="mb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {prodottiOrdinati.filter((p) => (p.regioni?.nome ?? "—") === regione).map((p) => (
                    <div key={p.id} className="rounded-xl p-4"
                      style={{ backgroundColor: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
                      <div className="flex justify-between items-start gap-2">
                        <div className="min-w-0">
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
            {[
              ...
              <div key={titolo} className="rounded-xl p-4"
                style={{ backgroundColor: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
