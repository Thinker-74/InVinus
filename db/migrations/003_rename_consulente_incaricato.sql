...
DROP FUNCTION IF EXISTS public.get_admin_kpi(integer, integer);
...
AS $$
  ...
  ORDER BY c.status DESC, COALESCE(p.pv,0) DESC;
$$;
...
-- ---------------------------------------------------------------------------
-- 11. FIX CHECK constraints e indici non coperti dal rename automatico
-- ---------------------------------------------------------------------------
ALTER TABLE eventi_partecipanti
  DROP CONSTRAINT eventi_partecipanti_tipo_partecipante_check,
  ADD CONSTRAINT eventi_partecipanti_tipo_partecipante_check
    CHECK (tipo_partecipante::text = ANY (ARRAY['lead','cliente','incaricato']::text[]));

ALTER TABLE interazioni_crm
  DROP CONSTRAINT interazioni_crm_tipo_soggetto_check,
  ADD CONSTRAINT interazioni_crm_tipo_soggetto_check
    CHECK (tipo_soggetto::text = ANY (ARRAY['lead','cliente','incaricato']::text[]));

ALTER TABLE cantina_personale
  DROP CONSTRAINT cantina_personale_utente_tipo_check,
  ADD CONSTRAINT cantina_personale_utente_tipo_check
    CHECK (utente_tipo::text = ANY (ARRAY['INCARICATO','CLIENTE']::text[]));

ALTER INDEX idx_prov_consulente_mese RENAME TO idx_prov_incaricato_mese;
